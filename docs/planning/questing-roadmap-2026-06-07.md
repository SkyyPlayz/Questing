...
## Addendum 2026-07-16: Mobile build decisions

The approved plan above is a backend stabilization / code-quality roadmap. It states the
product is mobile-first but does not describe how the mobile app is built or how it
connects to the backend. Those gaps are now filled by two companion documents, and the
following owner decisions (Gabe/Skyyplayz) are recorded:

- **Architecture:** keep Next.js as the backend + website; the mobile app is a separate
  client of the same API. Business logic is not rebuilt in the app.
- **Framework:** React Native (Expo). Build **iOS first, Android later** from one codebase.
- **Prerequisite:** the backend must become "mobile-ready" (token-based login instead of
  browser cookies, plus a stable versioned JSON API) before/alongside the iOS build.

Companion documents:

- `docs/planning/mobile-ios-plan.md` — the mobile build plan, API-readiness prerequisites,
  iOS/React Native parent issue groups, Stripe/App Store notes, and design handoff notes.
- `docs/planning/future-vision.md` — far-future, **not-yet-scoped** ideas, including the
  embedded "Quest Wallet" payments vision (in-app balance, linked bank accounts, cards,
  and same-day pay after a quest).
- `docs/planning/safety-escalation-plan.md` — how safety escalation must be built so the
  app can run its own system now and switch to a partner (e.g. Nightlight) later, and so
  that contacting police / emergency services can **never happen by accident** (simulate
  by default, multi-gated live dispatch, coordinated live-test protocol).

Note on the existing issue backlog: the open issues beyond #14 (currently ~#58–#110) were
generated by the owner's automated bug-hunt bots running on a recurring schedule while the
project sat idle; they are not a manual execution of this roadmap's rollout section. Treat
this roadmap's "recommended first batch" as historical context, not the current backlog.
